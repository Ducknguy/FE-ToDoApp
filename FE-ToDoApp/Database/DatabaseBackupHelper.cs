...
{
    /// <summary>
    /// Helper class ?? backup v� restore database
    /// ??c bi?t h?u �ch khi d�ng MSIX packaging
    /// </summary>
    public static class DatabaseBackupHelper
    {
        /// <summary>
        /// Backup database ra file ngo�i
        /// </summary>
        public static bool BackupDatabase(string targetPath = null)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Kh�ng t�m th?y database ?? backup!", "L?i", 
                ...
                MessageBox.Show($"Backup th�nh c�ng!\nFile ???c l?u t?i:\n{targetPath}", 
        ...
        public static bool RestoreDatabase(string sourcePath = null)
        {
            try
            {
                ...
                if (!File.Exists(sourcePath))
                {
                    MessageBox.Show("File backup kh�ng t?n t?i!", "L?i", 
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                var result = MessageBox.Show(
                    "Restore s? ghi ?� l�n database hi?n t?i!\n" +
                    "B?n c� ch?c ch?n mu?n ti?p t?c?\n\n" +
                ...
                // Copy file backup v�o v? tr� database
                File.Copy(sourcePath, targetPath, true);

                MessageBox.Show("Restore th�nh c�ng!\n?ng d?ng s? t?i l?i d? li?u.", 
                    "Th�nh c�ng", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        /// <summary>
        /// L?y th�ng tin v? database hi?n t?i
        /// </summary>
        ...
        public static void OpenDatabaseFolder()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Th? m?c database kh�ng t?n t?i!", "Th�ng b�o", 
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Kh�ng th? m? th? m?c:\n{ex.Message}", "L?i", 
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
